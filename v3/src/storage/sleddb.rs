...
impl Storage for SledDb {
    ...
    fn del(&self, table: &str, key: &str) -> Result<Option<Value>, KvError> {
        let name = SledDb::get_full_key(table, &key);
        let result = self.0.remove(name).unwrap().map(|v| v.as_ref().try_into());
        flip(result)
    }
...
/// 这里是自己加的，要不Ok(self.0.contains_key(name)?) 这种编译不过去。
impl From<sled::Error> for KvError {
    fn from(v: sled::Error) -> Self {
        KvError::Internal(v.to_string())
    }
}
